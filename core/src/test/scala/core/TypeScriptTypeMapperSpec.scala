...
final class TypeScriptTypeMapperSpec extends org.specs2.mutable.Specification {
  ...
  lazy val unresolved: Function4[Settings, String, TypeScriptField, TypeRef, String] = (_, _, _, _) => "_tpe_"
  ...
  "Mapper" should {
    ...
    "map nullable as Option" in {
      nullableAsOption(
        unresolved,
        settings,
        "_", member, NullableType(StringRef)) must beTypedEqualTo(
          Some("Option<_tpe_>"))
    }

    "map number as string" in {
      numberAsString(
        unresolved, settings, "_", member, NumberRef) must beSome("string")
    }
