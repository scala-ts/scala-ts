...
- `scalatsDeleteGeneratedSourceOnClean` - Boolean setting to delete the ScalaTS generated source directory on `clean` (default: `true` if the directory is within the target directory, otherwise `false`).
...
The following settings allow advanced customization of ScalaTS generation, including custom implementations of `TypeMapper`, `DeclarationMapper`, `ImportResolver`, and `Printer`.
...
If the Scala versions differ, these classes must be provided as cross versions Scala library to be appended to the `scalatsAdditionalClasspath` as below.
