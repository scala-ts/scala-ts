...
describe('Constants', () => {
  ...
  it('should declare complex values', () => {
    ...
    // Dictionary
    const dict: Readonly<Map<string, string>> = // Check inference
      ConstantsInhabitant.dict

    expect(dict.get('A')).toEqual("value #1")
    expect(dict.get('B')).toEqual(ConstantsInhabitant.name)

    expect((dict as any).get('C')).toEqual(undefined)

    // More complex list
    expect(ConstantsInhabitant.listOfDict instanceof Array).toBe(true)
    expect(ConstantsInhabitant.listOfDict.length).toEqual(2)

    const first: Readonly<Map<string, string>> | undefined =
      ConstantsInhabitant.listOfDict[0]

    expect(first).toEqual(new Map([ ['title', "Foo"], ['description', "..."]  ]))

    const second: Readonly<Map<string, string>> | undefined =
      ConstantsInhabitant.listOfDict[1]

    expect(second).toEqual(new Map([ ['title', "Bar"], ['description', "..."] ]))
