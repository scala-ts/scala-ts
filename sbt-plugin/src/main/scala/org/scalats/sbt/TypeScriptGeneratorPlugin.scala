...
object TypeScriptGeneratorPlugin extends AutoPlugin {
  ...
  override lazy val projectSettings = Seq(
    generateTypeScript := {
      ...
      val basedir = (sourceManaged in generateTypeScript).value

      TypeScriptGenerator.generateFromClassNames(
        config, args.toList, logger(streams.value.log),
        { name =>
          new PrintStream(new FileOutputStream(basedir / s"${name}.ts"))
        },
        cl)
    },
